...
void insert_name_list(sNAME_LIST *name_list,            // The linked list of names.
   ...
   if(name_list->front == NULL){          // List is empty
      name_list->front = node;            // Update front
      name_list->rear = node;             // Update rear
      name_list->count = 1;               // Update count
   }else{                                 // List not empty
      sNODE *n_ptr = name_list->front;    // Create pointer at front
      // Iterate through list, while new name is not found
      while(n_ptr != NULL && n_ptr != node){ 
         n_ptr = n_ptr->next;
      }
   }

   if(n_ptr == NULL){                     // Node not found in list
      name_list->rear->next = node;       // Insert at the end of list  
      name_list->rear = node;             // Update rear
      name_list->count++;
   }
}
