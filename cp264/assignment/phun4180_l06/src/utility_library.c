...
register stack *p;              // Local stack scan pointer.
 ...
 static bool initialize = TRUE;           // Static local variable "initialize" is set to TRUE ONLY the first time this
   ...
   // Move top of stack up to new top of stack (by STACK_VAL_LNG positions)
   p_tos = p_tos + STACK_VAL_LNG;
   // Top of stack is below stack max
   if (p_tos < p_mxs) {
      // Copy proc_name to top of stack
 ...
 //  NOTE: push() and pop() are not used in this utility routine in order to avoid an infinite calling loop.
 {
   ...
   for (i = 0; STACK_VAL_LNG - 2 > i; i++) {
       // Set top STACK_VAL_LNG - 1 elements to blank chars 
        *(&(*p_tos) + i) = cBLK;
   }
   // Set last element to NULL
   *(&(*p_tos) + STACK_VAL_LNG - 1) = cNUL;
   // Bring pointer to new top of stack (by STACK_VAL_LNG positions)
   p_tos = p_tos -  STACK_VAL_LNG;
   
   // Top of stack went under bottom of stack, underflow
   if (p_tos < p_bos) {
      // Update top of stack to also be bottom of stack
      p_tos = p_bos;
   }
