...
//================================================
{
	...
	// Check main cases
    if (abs(left_height - right_height) <= 1 &&						// Height check
        (node->left == NULL || node->left->data < node->data) &&	// Left node value check
        (node->right == NULL || node->right->data > node->data)) {	// Right node value check
        // Node is valid, check subtrees
        is_valid = avl_valid_aux(tree, node->left, min_node, node);
        if (is_valid) {
            is_valid = avl_valid_aux(tree, node->right, node, max_node);
        }
    }
