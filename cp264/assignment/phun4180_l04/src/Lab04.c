...
FILE  *fp_in        = NULL;           // Input file pointer.
   ...
   report_message(msg, MSG_END);           // Report end message
...
// The report_message() routine is called to output an appropriate "MSG_INFO" message if there are no lines of text to print.
{
   // Declare variables
   int i, j;
   char msg[MAX_MSG_NUM][MAX_MSG_LNG];


   // Fill 2D array with null chars
   for(i = 0; i < MAX_MSG_NUM; i++){
      for(j = 0; j < MAX_MSG_LNG; j++){
         msg[i][j] = cNUL;
      }
   }
   ...
   // Array is empty
      ...
      // Create message
      sprintf(msg[0], "%s", "There is no text to print.");
      // Report message 
...
static   char indent[]       = "       : ";
static   char end_prefix[]   = "==> END: Program Terminated. ";
   ...
   switch (msg_kind) {                                  // Switch on "msg_kind" to determine which message prefix to output!
      ...
      case MSG_END:
         ...
         printf(NL);
         printf(indent);
         printf("Have a great day.");  // Print formatted output for MSG_END
         break;
